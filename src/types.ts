...
export interface User {
  ...
  /**
   * `role` = papel no sistema (ADMIN, OPERADOR, etc.) — usado para controle de acesso.
   * Não confundir com `cargo`, que é a função profissional do colaborador (ex: "Motorista").
   */
  role: UserRole;
  /**
   * `cargo` = nome do cargo profissional do colaborador (ex: "Motorista de Caminhão").
   * É uma string livre oriunda da tabela `Cargo`, NÃO é o papel no sistema.
   * @see role para controle de acesso.
   */
  cargo: string | null;
  fotoUrl?: string | null;
...
export interface OrdemServico {
  ...
  // Aceita null para Equipementos/caixas (manutenção externa)
  kmAtual: number | null;
