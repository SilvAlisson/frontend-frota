...
import { treinamentosService, type CreateTreinamentoPayload, type CargoResponse, type Requisito } from '../services/modules/treinamentosService';
...
// ─────────────────────────────────────────────────────────────────
// Payload types removidos (agora importados)
...
export function useTreinamentosUsuario(userId: string, cargoId?: string | null) {
    ...
    const { data: cargos = [], isLoading: loadingCargos } = useQuery<CargoResponse[]>({
        ...
        queryFn: async () => {
            return await treinamentosService.getCargos();
        },
    ...
    const { data: rawTreinamentos = [], isLoading: loadingTreinamentos } = useQuery<TreinamentoRealizado[]>({
        queryKey,
        queryFn: async () => {
            return await treinamentosService.getByUserId(userId);
        },
        staleTime: 1000 * 60 * 2,
    });
    ...
    const addMutation = useMutation({
        mutationFn: (payload: CreateTreinamentoPayload) =>
            treinamentosService.create(payload),
        onSuccess: () => {
    ...
    const removeMutation = useMutation({
        mutationFn: (id: string) => treinamentosService.delete(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey });
            queryClient.invalidateQueries({ queryKey: ['users'] });
            queryClient.invalidateQueries({ queryKey: ['dossie-integrante'] });
    ...
    const importarPlanilha = async (file: File): Promise<number> => {
        const task = async (): Promise<number> => {
            ...
            await treinamentosService.importar(userId, valid);
