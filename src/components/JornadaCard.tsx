...
export function JornadaCard({
  ...
  if (isFinalizada) {
    return (
      <Card className={`p-8 flex flex-col items-center justify-center text-center transition-all bg-emerald-50 border-emerald-200`}>
        <div className={`p-4 rounded-full mb-4 shadow-inner bg-emerald-100 text-emerald-600`}>
          <CheckCircle2 className="w-8 h-8" />
        </div>

        <h3 className={`text-xl font-black tracking-tight text-emerald-700`}>
          Turno Encerrado
        </h3>
        ...
        <form className="space-y-6 relative z-10" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex flex-col mb-2">
            ...
            <p className="text-sm text-text-secondary font-medium mt-1">
              Registre o hodômetro de chegada do veículo <strong className="text-text-main font-mono bg-surface px-1.5 py-0.5 rounded border border-border/60">{jornada.veiculo?.placa || 'N/A'}</strong>.
            </p>
          </div>
