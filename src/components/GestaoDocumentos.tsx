...
export function FormCadastrarDocumento({ onSuccess, onCancel }: FormProps) {
  ...
  const handleUploadAndSubmit = async (data: DocFormValues) => {
    ...
    try {
      ...
      // Nome do arquivo sanitizado para evitar problemas de URL
      const fileName = `${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExt}`;
      const filePath = `documentos-legais/${fileName}`;

      // [CORREÇÃO CRÍTICA PARA MOBILE]
      // Define explicitamente o tipo do arquivo (PDF, JPEG, PNG)
      // Sem isso, o celular tenta baixar como binário desconhecido
      const { error: uploadError } = await supabase.storage
        .from('comprovantes')
        .upload(filePath, file, {
          cacheControl: '3600',
          upsert: false,
          contentType: file.type // <--- O SEGREDO ESTÁ AQUI
        });
  ...
  // Classes comuns para manter consistência com o Design System
  const inputClasses = "w-full h-11 px-3 bg-surface border border-border rounded-xl text-sm focus:border-primary outline-none text-text-main placeholder:text-text-muted transition-all disabled:bg-background disabled:text-text-muted appearance-none";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                {/* Ícone Chevron Manual para Select Nativo */}
