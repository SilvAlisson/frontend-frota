...
const produtoSchema = z.object({
  ...
  // CORREÇÃO: Usando spread operator para converter readonly array em array mutável
  tipo: z.enum([...tiposDeProduto]),
  unidadeMedida: z.string().min(1, "Unidade de medida obrigatória (ex: Litro, Un)."),
});
  ...
  useEffect(() => {
    ...
    const fetchProduto = async () => {
      ...
      try {
        ...
        const tipoValido = tiposDeProduto.includes(produto.tipo) ? produto.tipo : 'OUTRO';
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Button type="submit" variant="primary" className="flex-1" disabled={isSubmitting} isLoading={isSubmitting}>{isSubmitting ? 'Salvando...' : 'Alterações salvas!'}</Button>
      </div>
    </form>
  );
}