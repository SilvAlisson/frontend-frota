...
interface DashboardRelatoriosProps {
  veiculos: Veiculo[];
  // Callback opcional para quando clicar num card (Drill-down)
  onDrillDown?: (tipo: 'ABASTECIMENTO' | 'MANUTENCAO' | 'JORNADA' | 'GERAL') => void;
}

// === DESIGN DO CARD KPI (Atualizado com UX Interativa) ===
function KpiCard({ titulo, valor, descricao, corIcone, iconSvg, onClick }: {
  titulo: string,
  valor: string,
  descricao: string,
  corIcone: string,
  ...
  return (
    <div
      onClick={onClick}
      className={`
        bg-white rounded-card p-6 shadow-card border border-gray-100 flex items-start gap-4 
        transition-all duration-300 ease-out
        ${onClick ? 'cursor-pointer hover:-translate-y-1.5 hover:shadow-card-hover hover:border-primary/30 active:scale-95' : ''}
      `}
    >
      ...
      {kpis && !loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <KpiCard
            ...
            // Custo Total não leva a lugar específico ou leva para Geral
