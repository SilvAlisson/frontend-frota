...
export function ModalTreinamentosUsuario({ usuario, onClose }: ModalProps) {
    ...
    const executeDelete = async () => {
        ...
        toast.promise(promise, {
            ...
            error: () => {
                ...
                return 'Erro ao excluir Registro da base de dados.';
            }
        });
    };
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        toast.promise(importTask(), {
            ...
            error: (err) => {
                if (fileInputRef.current) fileInputRef.current.value = '';
                return err.message || "Falha na importação do Arquivo.";
            }
        });
    };
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300" onClick={onClose}>
            ...
            <ConfirmModal 
                ...
                description={`Tem certeza que deseja remover o Registro de "${treinoParaExcluir?.nome}" do histórico deste colaborador?`}
                variant="danger"
