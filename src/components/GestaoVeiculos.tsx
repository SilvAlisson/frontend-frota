          ...
          {modo === 'listando' && (
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <Button
                ...
                icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>}
              >
                Novo Veículo
              </Button>
            </div>
        ...
        <>
          {loading ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {[1, 2, 3, 4].map(i => <div key={i} className="h-48 bg-white rounded-xl border border-gray-100 animate-pulse"></div>)}
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                <div key={veiculo.id} className="group bg-white p-5 rounded-xl shadow-sm border border-gray-200 hover:shadow-float hover:border-primary/30 transition-all duration-300 flex flex-col relative">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <h4 className="font-mono font-bold text-lg text-gray-900 leading-none">{veiculo.placa}</h4>
                        <span className="text-[10px] text-gray-400 font-medium">ID DA FROTA</span>
                      </div>
                    </div>
                    <div className="pl-1">
                      <p className="text-sm font-medium text-gray-700 truncate" title={veiculo.modelo}>{veiculo.modelo}</p>
                      <p className="text-xs text-gray-500">Ano {veiculo.ano}</p>
                    </div>
                  </div>
                  <div className="mt-auto space-y-2 py-3 border-t border-gray-50">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      {(() => { const status = getDocStatus(veiculo.vencimentoCiv); return <span className={`px-2 py-0.5 rounded text-[10px] border ${status.classes}`}>{status.label}</span>; })()}
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-gray-400 font-bold uppercase text-[10px]">CIPP</span>
                      {(() => { const status = getDocStatus(veiculo.vencimentoCipp); return <span className={`px-2 py-0.5 rounded text-[10px] border ${status.classes}`}>{status.label}</span>; })()}
                    </div>
                  </div>
                  <div className="absolute inset-x-0 bottom-0 p-2 bg-white/95 backdrop-blur-sm border-t border-gray-100 flex justify-end gap-2 translate-y-full group-hover:translate-y-0 transition-transform duration-200">
                    <button onClick={() => { setVeiculoIdSelecionado(veiculo.id); setModo('editando'); }} className="p-2 text-gray-500 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors flex items-center gap-2 text-xs font-bold uppercase"><IconeEditar /> Editar</button>
                    <button onClick={() => handleDelete(veiculo.id)} disabled={deletingId === veiculo.id} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors flex items-center gap-2 text-xs font-bold uppercase">{deletingId === veiculo.id ? '...' : <><IconeLixo /> Excluir</>}</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {!loading && veiculos.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200 text-center">
              <div className="p-4 bg-white rounded-full mb-4 shadow-sm"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 text-gray-300"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" /></svg></div>
              <h4 className="text-gray-900 font-bold">Frota Vazia</h4>
              <p className="text-gray-500 text-sm mt-1 max-w-xs mx-auto">Comece cadastrando seus veículos para monitorar a operação.</p>
              <Button variant="ghost" onClick={() => setModo('adicionando')} className="mt-4 text-primary bg-primary/5 hover:bg-primary/10">Cadastrar Primeiro Veículo</Button>
            </div>
          )}
        </>
      )}
